...
app.get('/preview', async (req, res) => {
  ...
  console.log(`Generating preview for --> ${uri}`);

  if (cache.has(uri)) {
    console.log(`Success! Cache found preview for --> ${uri}`)
    res.send(cache.get(uri));
    return;
  }

  try {
      const previewData = await getPreview(uri);

      if (previewData === 'undefined') {
        console.error(`Internal server error when creating preview for --> ${uri}`);
        res.sendStatus(500);
        return;
      }

      console.log(`Success! Preview generated for --> ${uri}`)
      res.json(previewData);
      cache.set(uri, previewData);
    } catch (error) {
        console.log(error);
        res.send(error);
    }
...
process.on('SIGTERM', () => {
  console.log('Link preview server exiting');
  browser.close();
  server.close(() => {
    console.log('Link preview server closed');
  })
})